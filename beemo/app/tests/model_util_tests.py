...
from app.models import Participant, Call
from app.models import calculate_adherence_score, calculate_veg_servings_score, calculate_fruit_servings_score
from app.models import calculate_fiber_grams_score, calculate_fat_grams_score, calculate_steps_score
...
class AdherenceScoreTestCase(ModelUtilTestCase):
    ...
    def test_should_return_not_none_when_all_values_present(self):
        ...
        self.assertIsNotNone(calculate_adherence_score(self.participant, self.call))
    ...
    def test_should_return_none_when_single_value_missing(self):
        ...
        self.assertIsNone(calculate_adherence_score(self.participant, self.call))
    ...
    def test_should_calculate_correct_fat_scores(self):
        ...
        for value in range(1,50):
            ...
            if percentage >= 1.4:
                self.assertEqual(calculate_fat_grams_score(value, base_value), 0, 
                    'wrong fat score value for value %s and base value %s' % (value, base_value))
            elif percentage >= 1.2:
                self.assertEqual(calculate_fat_grams_score(value, base_value), 1, 
                    'wrong fat score value for value %s and base value %s' % (value, base_value))
            elif percentage >= 1.1:
                self.assertEqual(calculate_fat_grams_score(value, base_value), 3, 
                    'wrong fat score value for value %s and base value %s' % (value, base_value))
            else:
                self.assertEqual(calculate_fat_grams_score(value, base_value), 5, 
                    'wrong fat score value for value %s and base value %s' % (value, base_value))
    ...
    def test_should_calculate_correct_step_scores(self):
        ...
        for value in range(0,10):
            ...
            if difference > 2000:
                self.assertEqual(calculate_steps_score(value * 500, base_value), 0,
                    'wrong step score value for value %s and base value %s' % (value * 500, base_value))
            elif difference > 1000:
                self.assertEqual(calculate_steps_score(value * 500, base_value), 1,
                    'wrong step score value for value %s and base value %s' % (value * 500, base_value))
            elif difference > 1:
                self.assertEqual(calculate_steps_score(value * 500, base_value), 3,
                    'wrong step score value for value %s and base value %s' % (value * 500, base_value))
            else:
                self.assertEqual(calculate_steps_score(value * 500, base_value), 5,
                    'wrong step score value for value %s and base value %s' % (value * 500, base_value))
